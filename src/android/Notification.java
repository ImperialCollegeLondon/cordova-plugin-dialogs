...
public class Notification extends CordovaPlugin {
	...
	/**
	 * Builds and shows a native Android prompt dialog with given title,
	 * message, buttons. This dialog only shows up to 3 buttons. Any labels
	 * after that will be ignored. The following results are returned to the
	 * JavaScript callback identified by callbackId: buttonIndex Index number of
	 * the button selected input1 The text entered in the prompt dialog box
	 * 
	 * @param message
	 *            The message the dialog should display
	 * @param title
	...
	public synchronized void activityStart(final String title, final String message) {
		...
		Log.v("CORDOVA","no message to display");
		
		final CordovaInterface cordova = this.cordova;
		Runnable runnable = new Runnable() {
			public void run() {

				// if the message is empty just show a spinner loader, no
				// message, no box
